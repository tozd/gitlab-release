...
func Sync(client *gitlab.Client, baseURL, projectID string, release Release, milestones []string, packages []Package, images []string) errors.E {
	name := release.Tag
	if release.Yanked {
		name += " [YANKED]"
	}
...
func MapImagesToTags(images []string, releases []Release) map[string][]string {
	...
	// First we do a regular sort, so that we get deterministic results later on.
	sort.Stable(sort.StringSlice(tags))
	sort.Stable(sort.StringSlice(images))
	// Then we sort by length, so that we can map longer tag names first
	// (e.g., 1.0.0-rc before 1.0.0).
	sort.SliceStable(tags, func(i, j int) bool {
		return len(tags[i]) > len(tags[j])
	...
	assignedImages := mapset.NewThreadUnsafeSet()
	for _, removePrefix := range []bool{false, true} {
		for _, tag := range tags {
			t := tag
			if removePrefix {
			...
			for _, image := range images {
				if assignedImages.Contains(image) {
					continue
				}

				if strings.Contains(image, t) {
					if tagsToImages[tag] == nil {
						tagsToImages[tag] = []string{}
					}
					tagsToImages[tag] = append(tagsToImages[tag], image)
...
func SyncAll(config Config) errors.E {
	...
	for _, release := range releases {
		err = Sync(client, config.BaseURL, config.Project, release, tagsToMilestones[release.Tag], tagsToPackages[release.Tag], tagsToImages[release.Tag])
		if err != nil {
			return err
		}
	}
